...
The UI is **React** on **Vite** in TypeScript, with layout and typography in plain CSS. Icons come from **Lucide** where it fits; a couple of brand marks are inlined where the icon set does not ship them. Content lives in code so the whole thing stays easy to revise and deploy as static files: `lib/data/` holds the language-independent structure (app links, icons, accents, category keys), and `lib/i18n/` holds every user-facing string.
